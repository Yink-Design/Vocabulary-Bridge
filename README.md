...
墨墨开放 API 目前没有提供“当前正在学习的官方词书”以及“某个单词是否属于该官方词书”的查询接口，因此 Vocabulary Bridge 使用本地词书白名单。
...
- 词书内 → 继续执行智能学习路由。
- 词书外 → 停止调用学习 API，并显示：`为词书外单词`。

未导入词书白名单时，程序会明确显示“当前词书过滤未配置”，不会伪造词书判断。
...
- 右下角系统托盘常驻，可手动捕获、导入当前词书、更新 Token 或退出。
...
开放 API 当前没有官方词书查询能力，因此需要一次性导入当前词书白名单。程序不会调用非官方私有接口去猜测当前词书。
